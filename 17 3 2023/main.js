...
function size_error(){
   ...
   if(!str) err=" Please fill this field.";
      ...
      if(value<5 || value>300) err=" Size should be in given Range.";
      else{
         custom_size = value;
         return true;
      }
...
function generate_new_array(){
   ...
   if(!is_valid) custom_array_btn.className="btn btn-outline-light my-2 my-sm-1"
   // $('.alert').alert('close');
   change_speed();
   for(let i=0;i<total_size;i++){
      ...
      apply_style(bar_div[i]," white",bar_len[i]);
   }
}

function change_speed(){
   Aspeed.innerHTML="Speed(" + Math.pow(2,parseInt(req_speed.value)) + "X)";
